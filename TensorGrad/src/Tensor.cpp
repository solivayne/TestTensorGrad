...
Tensor::Tensor(
    ...
    bool isParam) : shape(shape), strides(strides), device(Device::CPU)
{
...
Tensor Tensor::operator+(const Tensor &other) const
{
    if (this->device != other.device)
        throw std::runtime_error("Tensors must be on same device to add.");
...
#pragma omp parallel for
    ...
    {
        if (is_cuda)
            throw std::runtime_error("GPU backward pass not yet implemented.");
...
{
    if (this->device != other.device)
        throw std::runtime_error("Tensors must be on same device to subtract.");
    if (this->device == Device::CUDA)
        throw std::runtime_error("CUDA kernel for subtraction not yet implemented.");
...
{
    if (this->device == Device::CUDA)
        throw std::runtime_error("CUDA kernel for pow not yet implemented.");
...
{
    if (this->device == Device::CUDA)
        throw std::runtime_error("CUDA kernel for sum not yet implemented.");
...
{
    if (this->device == Device::CUDA)
        throw std::runtime_error("CUDA kernel for relu not yet implemented.");
